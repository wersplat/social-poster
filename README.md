...
| `POG_SUPERHERO_MODE` | Enable superhero-themed **Instagram** `player_of_game` graphics (`true`/`false`, default `false`). When on, the image prompt includes mood caption plus canonical player/team/stat line/deduped quote for scene alignment; the render reserves a single bottom band for text. Playwright composites one bottom panel (kicker, name, team, stats, quote; hashtags stripped in-quote), plus league logo and date. When off, the abstract spotlight plate + standard POG overlay is used. Falls back if no caption is available. |
