...
| `POG_SUPERHERO_MODE` | Enable superhero-themed **Instagram** `player_of_game` graphics (`true`/`false`, default `false`). When on, the AI image generator uses the same superhero prompt (caption mood + comic art, empty speech bubbles/panels, no readable player stats in-image). Playwright composites name, stat line, team, caption (hashtags stripped on-image), league logo, and date. When off, the existing abstract spotlight plate + full Playwright text overlay is used. Falls back to the abstract plate if no caption is available. |
